...
namespace RPG
{
    public class PlayerMovement2D : MovingObject
    {
        ...
        void Update()
        {
            ...
            if ((horizontal = (int)CrossPlatformInputManager.GetAxisRaw("Horizontal")) != 0)
            {
                lastMove.x = horizontal;
                lastMove.y = 0;
            }

            if ((vertical = (int)CrossPlatformInputManager.GetAxisRaw("Vertical")) != 0)
            {
                lastMove.y = vertical;
                lastMove.x = 0;
            }
            ...
            anim.SetFloat(intHashHorizontal, horizontal);
            anim.SetFloat(intHashVertical, vertical);
